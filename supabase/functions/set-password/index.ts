// Supabase Edge Function: set-password
// Lets an ADMIN set a new password for a user (e.g. when an installer can't log in).
// Verifies the caller is an admin, then uses the service role to update the password.
// Returns the password so the admin can share it.

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
}

serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders })
  }

  try {
    const SUPABASE_URL = Deno.env.get('SUPABASE_URL')!
    const SERVICE_ROLE = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const ANON_KEY = Deno.env.get('SUPABASE_ANON_KEY')!

    // 1) Verify the caller is a signed-in admin
    const authHeader = req.headers.get('Authorization') || ''
    const callerClient = createClient(SUPABASE_URL, ANON_KEY, {
      global: { headers: { Authorization: authHeader } },
    })
    const { data: { user: caller }, error: callerErr } = await callerClient.auth.getUser()
    if (callerErr || !caller) return json({ error: 'Not authenticated' }, 401)
    const { data: callerProfile } = await callerClient
      .from('profiles').select('role').eq('id', caller.id).single()
    if (callerProfile?.role !== 'admin') return json({ error: 'Admin access required' }, 403)

    // 2) Input
    const body = await req.json().catch(() => ({}))
    const userId = (body.user_id || '').trim()
    if (!userId) return json({ error: 'user_id is required' }, 400)
    const password = (body.password || '').trim() || generatePassword()
    if (password.length < 8) return json({ error: 'Password must be at least 8 characters' }, 400)

    // 3) Update the password + ensure the email is confirmed so they can log in
    const admin = createClient(SUPABASE_URL, SERVICE_ROLE)
    const { error } = await admin.auth.admin.updateUserById(userId, { password, email_confirm: true })
    if (error) return json({ error: error.message }, 400)

    return json({ success: true, user_id: userId, password }, 200)
  } catch (e) {
    return json({ error: String(e) }, 500)
  }
})

function json(payload: unknown, status = 200) {
  return new Response(JSON.stringify(payload), {
    status,
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  })
}

function generatePassword(): string {
  const bytes = new Uint8Array(9)
  crypto.getRandomValues(bytes)
  const base = btoa(String.fromCharCode(...bytes)).replace(/[+/=]/g, '')
  return 'Mg' + base.slice(0, 9) + '7!'
}
